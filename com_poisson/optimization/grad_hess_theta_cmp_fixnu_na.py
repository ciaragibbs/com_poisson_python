"""Gradient and Hessian of the CMP log-posterior with fixed nu."""
...
def gradHessTheta_CMP_fixNu_na(
    vecTheta: np.ndarray,
    X_lam: np.ndarray,
    nu: float,
    ...
    """Compute gradient and Hessian of the CMP log-posterior with fixed nu.
    ...
    for t in range(T):
        if not np.isnan(spk_vec[t]):
            lam = float(np.exp(X_lam[t, :] @ theta[:, t]))
            mean_Y, var_Y, _, _, _, _ = CMPmoment(lam, nu, maxSum)

            SCORE[:, t] = (spk_vec[t] - mean_Y) * X_lam[t, :]
            x_t = X_lam[t, :, np.newaxis]
            hess = -var_Y * (x_t @ x_t.T)
        else:
            SCORE[:, t] = 0.0
